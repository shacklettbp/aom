...
static void rd_pick_sb_modes(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  if (aq_mode == RDO_AQ) {
    av1_set_block_thresholds(cm, &cpi->rd, 1.0);
  }
...
static void rd_use_partition(AV1_COMP *cpi, ThreadData *td,
  ...
  switch (partition) {
    ...
    case PARTITION_SPLIT:
      ...
      for (i = 0; i < 4; i++) {
        int x_idx = (i & 1) * mi_step;
        int y_idx = (i >> 1) * mi_step;
        int jj = i >> 1, ii = i & 0x01;
        RD_COST tmp_rdc;
...
static void rd_pick_partition(AV1_COMP *cpi, ThreadData *td,
  ...
  if (partition_none_allowed) {
    ...
    if (this_rdc.rate != INT_MAX) {
      if (bsize >= BLOCK_8X8) {
        ...
        this_rdc.rdcost += SCALED_RATE(x->rdmult, cpi->partition_cost[pl][PARTITION_NONE]);
      }
  ...
  if (do_split) {
    ...
    if (sum_rdc.rdcost < best_rdc.rdcost && i == 4) {
      ...
      sum_rdc.rdcost += SCALED_RATE(x->rdmult, sum_rdc.rate);
